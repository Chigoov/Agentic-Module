...
`build_phase` di config/kode tetap marker runtime legacy sampai diganti oleh
kontrak status roadmap yang benar-benar dibutuhkan.
...
| 7 | Research Agents | Bangun TaskAnalyzerAgent, ResearchPlannerAgent, DiscoveryAgent, VerificationAgent, RetrievalAgent, EvidenceAgent, ClaimAgent. | Concrete research agents dengan input/output contract dan tests. | ▶ NEXT |
| 8 | Orchestrator | Bangun pengatur alur yang menghubungkan agent dan tool, termasuk retry, failure loop, dan human review gate. | Workflow engine/orchestrator, retry policy, review gate. | ⬜ RENCANA |
| 9 | Model Provider & Routing | Aktifkan ModelRouter dan provider abstraction; dukung 9Router sebagai salah satu provider (bukan dependency wajib); fallback + telemetry token. | Provider interfaces, routing policy, fallback, token/cost telemetry. | ⬜ RENCANA |
...
| 12 | Citation + Fact Audit | Jalankan CitationAudit dan FactAudit pada draft. | citation_audit.json, fact_audit.json, revision loop. | 🔶 SEBAGIAN |
| 13 | DOCX Generation | Konversi draft yang lolos audit menjadi dokumen Word dengan format sitasi yang dapat dikonfigurasi. | final.docx, format APA 7 default + style lain. | ⬜ RENCANA |
| 14 | Academic Writing Mode | Satukan pipeline ringan untuk tugas sehari-hari (makalah, artikel, PKM, proposal). | End-to-end Academic Writing workflow. | ⬜ RENCANA |
| 15 | Deep Research Mode | Aktifkan multi-query, multi-source, dedup, deeper verification, full-text, conflict handling, synthesis mendalam. | End-to-end Deep Research workflow. | ⬜ RENCANA |
| 16 | End-to-End Validation | Uji seluruh sistem dengan kasus nyata (sederhana, teori klasik, literatur terbaru, DOI palsu, evidence bertentangan). | E2E test reports, failure-case evidence, readiness decision. | ⬜ RENCANA |
| 17 | Optimization | Optimalkan token, context, model routing, caching, concurrency, search efficiency, reliability, cost. | Performance metrics, token telemetry, routing/budget tuning, optimization report. | ⬜ RENCANA |
...
>
> **Urutan tidak murni linear.** Fase 10–11 sudah SELESAI sementara 6–9 belum seluruhnya selesai. Tabel status adalah sumber kebenaran roadmap, bukan satu integer `build_phase`.
